// 你只需要改這個檔案就能新增/修改藝術家
// image 路徑建議：images/artists/xxx.jpg
// slug 會用來產生網址：artist.html?slug=xxx

window.ARTISTS = [
  {
    slug: "a",
    name: "夏陽",
    role: "1932－2022",
    keywords: ["節奏", "動勢", "空間感"],
    cardBio: "臺灣現代藝術發展中極具代表性的重要藝術家，亦為「東方畫會」核心成員之一。",
    image: "images/artists/夏陽.jpg",
    detail: {
      intro: "夏陽（1932－2022）是臺灣現代藝術發展中極具代表性的重要藝術家，亦為「東方畫會」核心成員之一。他出生於中國江蘇，1949年後來臺，早年接受學院派訓練，並於1950年代投身臺灣現代藝術運動，積極推動抽象與前衛藝術在地化的發展。
夏陽的創作橫跨繪畫、雕塑與裝置，風格多元而持續轉變，展現高度實驗精神。1960年代起，他以抽象表現為主要方向，融合西方現代主義語彙與東方書寫性線條，強調節奏、動勢與空間感。其後期作品逐漸轉向符號化與童稚意象，經常運用動物、人物與簡化圖像，構築介於遊戲性與哲學思考之間的視覺世界。
在藝術觀念上，夏陽主張藝術應回應當代生活經驗，而非僅停留於形式探索。他以自由而幽默的創作態度，打破抽象與具象的界線，反映對文化、社會與人性狀態的深層關注。作為臺灣現代藝術的重要推手，夏陽不僅以作品奠定其歷史地位，也為戰後臺灣藝術開拓了更為開放與多元的創作視野。"}
  },
  {
    slug: "b",
    name: "Anthony Gormley<br>（安東尼・葛姆雷）",
    role: "1950－",
    keywords: ["測量空間", "感知世界"],
    cardBio: "英國當代最具國際影響力的雕塑藝術家之一，其創作長期圍繞「身體、空間與存在」的關係展開。",
    image: "images/artists/Anthony-Gormley.jpg",
    detail: {
      intro: "【這裡貼：藝術家詳細介紹第一段】"}
  },
    {
    slug: "c",
    name: "林永政（3cm）",
    role: "??",
    keywords: ["超現實感", "自我認同"],
    cardBio: "來自臺灣台南的當代攝影藝術家，以獨特、帶有超現實感的身體影像聞名。",
    image: "images/artists/林永政.jpg",
    detail: {
      intro: "【這裡貼：藝術家詳細介紹第一段】"}
  },
    {
    slug: "d",
    name: "王尋",
    role: "1962-",
    keywords: ["創新雕塑", "數位創意"],
    cardBio: "美籍臺裔當代雕塑家，以融合傳統雕刻技藝與現代數位科技的跨世代創作著稱。",
    image: "images/artists/王尋.jpg",
    detail: {
      intro: "【這裡貼：藝術家詳細介紹第一段】"}
  },
    {
    slug: "e",
    name: "Rona Pondick<br>（羅娜・龐迪克）",
    role: "1952－",
    keywords: ["後極簡主義", "身體政治"],
    cardBio: "美國重要的當代女性雕塑藝術家，以探討身體、心理狀態與存在焦慮聞名。",
    image: "images/artists/Rona-Pondick.jpg",
    detail: {
      intro: "【這裡貼：藝術家詳細介紹第一段】"}
  },
    {
    slug: "f",
    name: "Louise Bourgeois<br>（露易絲・布爾喬亞）",
    role: "1911–2010",
    keywords: ["心理創傷"],
    cardBio: "20世紀至當代藝術史中極具影響力的藝術家之一，其創作橫跨雕塑、裝置、繪畫與版畫，長期圍繞記憶、情感、性別、身體與心理創傷等主題。",
    image: "images/artists/Louise-Bourgeois.jpeg",
    detail: {
      intro: "【這裡貼：藝術家詳細介紹第一段】"}
  },
    {
    slug: "g",
    name: "Frederic Fontenoy<br>（弗雷德里克・方特努瓦）",
    role: "1964－",
    keywords: ["記憶", "感知", "存在"],
    cardBio: "法國當代藝術家，以描繪模糊、殘影般的人體形象聞名，其創作關注記憶、感知與存在狀態之間的曖昧關係。",
    image: "images/artists/Frederic-Fontenoy.jpg",
    detail: {
      intro: "【這裡貼：藝術家詳細介紹第一段】"}
  },
    {
    slug: "h",
    name: "Henrietta Harris<br>（亨麗埃塔·哈里斯）",
    role: "1984—",
    keywords: ["超現實", "解讀情感"],
    cardBio: "當代極具辨識度的肖像畫家與插畫藝術家，作品融合超現實、細膩技法與心理深度。",
    image: "images/artists/Henrietta-Harris.jpg",
    detail: {
      intro: "【這裡貼：藝術家詳細介紹第一段】"}
  }
];
